...
let text = `
# JavaScript 如何生成二维零矩阵数组？

## 〇、楔子

- 逛论坛的时候偶然看到了这个问题：如何快速生成一个二维 n\*m 且全为 0 的矩阵？看着这个问题，我陷入了沉思，顺便借着这次机会就对数组的知识进行了一系列的学习\~。

## 一、快速生成二维零矩阵

- 如何快速生成一个二维 n\*m 且全为 0 的矩阵？当时看到这个问题，我就啪，很快啊，写出了一行代码：
...
- 这是为什么呢？带着满脑子的问号，我去 MDN 翻了翻关于 \`fill()\`方法的资料，看到描述中有这样一句话：
...
- 既然前边说了如何生成二维 0 矩阵的数组，那我们可以扩展一下这个问题——如何生成多维零矩阵数组呢？
...
- 这样就可以进行生成简单的多维数组了，经过粗略的自测后也没有发现某一个子数组被引用的情况 XD，最终效果如下：
...
// 初始化pinia
export const useArticlePageStore = defineStore('articlePage', () => {
    // state
    const articleDetail = ref<any>({});
    const getArticleDetailState = computed(() => articleDetail.value);

    // action
    const getArticleDetailAction = async (id: number) => {
